...
LATTEDRONE_IP='10.30.5.21'
SOURCE_WS_PATH=~/sources/ros_workspaces/lattedrone_ws
CONTAINER_PATH=~/docks/ros_lattedrone
DOCKER_WS_PATH=${CONTAINER_PATH}/ros_ws
REMOTE_PATH=\~/ros_docker_noetic/ros_ws
...
synched_sources=`rsync -rlt --stats --human-readable --exclude=".*" --delete-delay ${SOURCE_WS_PATH}/src/. ${DOCKER_WS_PATH}/src/. | fgrep 'Number of regular files transferred' | cut -d' ' -f6 | tr -d ,`
...
build_errors=`${CONTAINER_PATH}/exec_no_it.sh "cd ~/catkin_ws && catkin build" >/dev/null`
if [ $? -ne 0 ]; then
	echo $build_errors
...
uploaded_files=`rsync -rlt --stats --human-readable --exclude=".*" --delete-delay ${DOCKER_WS_PATH}/. percro@${LATTEDRONE_IP}:${REMOTE_PATH}/. | fgrep 'Number of regular files transferred' | cut -d' ' -f6 | tr -d ,`
